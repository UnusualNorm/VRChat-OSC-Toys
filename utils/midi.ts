// TODO: Get these functions outa' here!
export function range(start: number, stop?: number, step?: number) {
  ...
  const increasing = start < stop; //← here’s the change
...
export function difference(array: number[], ...values: number[][]) {
  const rest = new Set(values.flat());
  return array.filter((value) => !rest.has(value));
}
...
function fromNote(note: string) {
  ...
  const pitchName =
    `${letter.toUpperCase()}${accidental}` as keyof typeof PITCH_INDEXES;
